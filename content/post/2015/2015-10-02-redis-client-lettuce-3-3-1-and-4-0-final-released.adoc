+++
title = "Redis Client lettuce 3.3.1 and 4.0 Final released"
date = "2015-10-02T13:48:28"
lastmod = "2025-01-03T22:48:49"
draft = false
slug = "redis-client-lettuce-3-3-1-and-4-0-final-released"
wordpress_id = "229"
url = "/2015/10/02/redis-client-lettuce-3-3-1-and-4-0-final-released/"
categories = ["Blog"]
+++
I just released lettuce https://github.com/mp911de/lettuce/releases/tag/3.3.1.Final[3.3.1.Final] and lettuce 4.0.Final. I'm going to tell you in this post more about 4.0. You can find the full details about https://github.com/mp911de/lettuce/releases/tag/3.3.1.Final[3.3.1.Final] and https://github.com/mp911de/lettuce/releases/tag/4.0.Final[4.0.Final] on Github.
...
This release contains some breaking changes. You may want to consult the wiki at https://github.com/mp911de/lettuce/wiki/Migration-from-3.x-to-4.x[Migration from 3.x to 4.x] to check the migration guide.
...
Observable observable = reactiveApi.keys("*");
...
Read more: https://github.com/mp911de/lettuce/wiki/Reactive-API-(4.0)[Reactive API]
...
// still working
RedisAsyncConnection connection = client.connectAsync();
----
...
See https://github.com/mp911de/lettuce/wiki/Migration-from-3.x-to-4.x#optional-changes[Migration from 3.x to 4.x] for the migration matrix.
...
NodeSelection<String, String> sync = connection.slaves();
...
Read more: https://github.com/mp911de/lettuce/wiki/Redis-Cluster-(4.0)[Redis Cluster]
...
Read more: https://github.com/mp911de/lettuce/wiki/ReadFrom-Settings[ReadFrom Settings]
...
Read more: https://github.com/mp911de/lettuce/wiki/Custom-commands%2C-outputs-and-command-mechanics[Custom commands&#44; outputs and command mechanics]

== Codec API improvements

The RedisCodec API was aligned to a consistent interchange type and migrated to an interface. +
The RedisCodec interface accepts and returns `ByteBuffer` for data interchange. A `ByteBuffer` is not opinionated about the source of the underlying bytes and such it does not require users to duplicate existing data as it was enforced by a `byte++[]++`. Lettuce provides `UTF-8` and `byte++[]++` codecs and allows users to create and use their own codecs. XML, JSON and Java-serialization are good examples for codecs.

This release also brings a value compressor which allows you to compress values using GZIP or Deflate compressions. Value compression is transparent and can be used with any codec.

[source,java]
----
RedisCommands<String, Object> connection = client.connect(
           CompressionCodec.valueCompressor(new SerializedObjectCodec(), GZIP)).sync(); 

RedisCommands<String, String> connection = client.connect(
              CompressionCodec.valueCompressor(new Utf8StringCodec(), DEFLATE)).sync(); 
----

Read more: https://github.com/mp911de/lettuce/wiki/Codecs-%284.x%29[Codecs]
...
* Provide a sync API for Redis Sentinel https://github.com/mp911de/lettuce/issues/79[++#++79]
* Remove Java 6/7 parts of JavaRuntime and use netty's SslContextBuilder https://github.com/mp911de/lettuce/issues/85[++#++85]
* Support geo commands in lettuce 4.0 https://github.com/mp911de/lettuce/issues/87[++#++87]
* Run certain commands from the advanced cluster connection on multiple hosts https://github.com/mp911de/lettuce/issues/106[++#++106]
* Allow limiting the request queue size https://github.com/mp911de/lettuce/issues/115[++#++115]
* Improve Codec API https://github.com/mp911de/lettuce/issues/118[++#++118]
* Allow to read from master/slave/nearest node when using Redis Cluster https://github.com/mp911de/lettuce/issues/114[++#++114]
* Documentation of custom commands https://github.com/mp911de/lettuce/issues/122[++#++122]
* Implement a CompressionCodec for GZIP and Deflate compression https://github.com/mp911de/lettuce/issues/127[++#++127]
* Implement synchronous multi-node execution API https://github.com/mp911de/lettuce/issues/129[++#++129]
* Use LinkedHashMap instead of HashMap for Map outputs https://github.com/mp911de/lettuce/issues/133[++#++133] (thanks to https://github.com/longkerdandy[@longkerdandy])
* Implement client factory methods (RedisClient.create, RedisClusterClient.create) https://github.com/mp911de/lettuce/issues/136[++#++136]

== Fixes

* CI: Build ran into OutOfMemoryError: Java heap space https://github.com/mp911de/lettuce/issues/84[++#++84]
* Use Sucess instead of Void in the reactive API https://github.com/mp911de/lettuce/issues/128[++#++128]
* GEOADD passes long/lat parameters in the wrong order to Redis https://github.com/mp911de/lettuce/issues/134[++#++134] (thanks to https://github.com/IdanFridman[@IdanFridman])
* Strip username from URI userinfo when creating a RedisURI with a username https://github.com/mp911de/lettuce/issues/131[++#++131] (thanks to https://github.com/jsiebens[@jsiebens])
* GeoArgs not evaluated when calling georadiusbymember(...) https://github.com/mp911de/lettuce/issues/142[++#++142]
* Guava's CacheBuilder missing in shaded jar https://github.com/mp911de/lettuce/issues/143[++#++143]

== Other

* Documentation for 4.0 https://github.com/mp911de/lettuce/issues/83[++#++83]
* Improve performance in 4.0 https://github.com/mp911de/lettuce/issues/91[++#++91]
* Update Dependencies for lettuce 4.0 https://github.com/mp911de/lettuce/issues/116[++#++116]
* Documentation of custom commands https://github.com/mp911de/lettuce/issues/122[++#++122]
* Client-specific client options and reduce visibility of AbstractClient.setOptions https://github.com/mp911de/lettuce/issues/135[++#++135]
...
* https://github.com/mp911de/lettuce/releases/tag/3.3.1.Final[Release 3.3.1.Final on Github]
* https://github.com/mp911de/lettuce/releases/tag/4.0.Final[Release 4.0.Final on Github]
...
* http://search.maven.org/remotecontent?filepath=biz/paluch/redis/lettuce/3.3.1.Final/lettuce-3.3.1.Final.jar[Download lettuce 3.3.Final]
* http://search.maven.org/remotecontent?filepath=biz/paluch/redis/lettuce/4.0.Final/lettuce-4.0.Final.jar[Download lettuce 4.0.Final]
* https://github.com/mp911de/lettuce/wiki[Wiki]
* http://redis.paluch.biz/docs/api/releases/3.3.1.Final/[Javadoc 3.3.1.Final]
* http://redis.paluch.biz/docs/api/releases/4.0.Final/[Javadoc 4.0.Final]
